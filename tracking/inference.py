...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for legalLocation in self.legalPositions:
                trueDistance = util.manhattanDistance(legalLocation, pacmanPosition)
                allPossible[legalLocation] = emissionModel[trueDistance] * self.beliefs[legalLocation]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
            newDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, oldPosition))
            for newPosition, probability in newDistribution.items():
                allPossible[newPosition] = allPossible[newPosition] + probability * self.beliefs[oldPosition]
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            (all_possible, belief) = util.Counter(), self.getBeliefDistribution()
            for position in self.legalPositions:
                dist = util.manhattanDistance(position, pacmanPosition)
                all_possible[position] += emissionModel[dist] * belief[position]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPosition in self.particles:
            newPosition = self.getPositionDistribution(self.setGhostPosition(gameState, oldPosition))
            temp.append(util.sample(newPosition))
        self.particles = temp
